...
class BaseTrainer(ABC):
    ...
    def _convert_dataset_to_dict(cls, dataset: Dataset):
        ...
        elif isinstance(dataset, datasets.CelebA):
            output_dict["type"] = "CELEBA"
            output_dict["args"]["root"] = dataset.root
            output_dict["args"]["split"] = dataset.split
            output_dict["args"]["transform"] = cls._convert_transforms_to_dict(dataset.transform)
    ...
    def _convert_dict_to_dataset(cls, dataset_dict: Dict[str, Any]):
        ...
        if dataset_dict["type"] == "ImageFolder":
            ...
            dataset_args["transform"] = cls._convert_dict_to_transforms(dataset_args["transform"])
        elif dataset_dict["type"] == "CELEBA":
            dataset_function = datasets.CelebA
            dataset_args["transform"] = cls._convert_dict_to_transforms(dataset_args["transform"])
    ...
    def _copy_file_to_dict(self, src_file: str, dest_dict: str):
        shutil.copyfile(src_file, os.path.join(dest_dict, os.path.basename(src_file)))

    def _save_backup_python(self, save_path: str):
        copy_path = os.path.join(save_path, "backup")
        
        self._copy_file_to_dict(inspect.getsourcefile(self.__class__), copy_path)
        self._copy_file_to_dict(inspect.getsourcefile(BaseTrainer), copy_path)
        self._copy_file_to_dict(inspect.getsourcefile(BaseGenerator), copy_path)
        self._copy_file_to_dict(inspect.getsourcefile(BaseDiscriminator), copy_path)

        self._copy_file_to_dict(inspect.getsourcefile(self.generator.__class__), copy_path)
        self._copy_file_to_dict(inspect.getsourcefile(self.discriminator.__class__), copy_path)
    ...
    def train(
        ...
        os.makedirs(os.path.join(save_path, "backup"), exist_ok=True)
        ...
        logger.Info("Saving backup of python files")
        self._save_backup_python(save_path)
        ...
        for epoch in range(start_epoch, training_config.epochs + 1):
            ...
            for batch_index, (real_images, real_labels) in enumerate(tqdm.tqdm(dataloader, unit="batch")):
                ...
                for discriminator_iterations in range(0, training_config.discriminator_repeats):
                    ...
                    discriminator_losses = self._train_discriminator_iteration(real_images.to(device=self.device), fake_images.detach(), should_zero_grad, epoch, real_labels.to(device=self.device), fake_labels)
                ...
                generator_losses = self._train_generator_iteration(epoch, should_zero_grad)
            ...
            with torch.no_grad():
                if epoch % training_config.sample_epochs == 0 or epoch == training_config.epochs:
                    sample_visualizations = self.generator(test_latents.detach(), test_labels).cpu().permute(0, 2, 3, 1)
